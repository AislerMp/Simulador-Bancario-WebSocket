...
export async function getCuentaByNumero(numeroCuenta) {
  ...
  const result = await pool
    ...
    .input("numero_cuenta", sql.VarChar(22), numeroCuenta)
    .query(
      "SELECT TOP 1 id_cuenta FROM Cuenta_Bancaria WHERE numero_cuenta = @numero_cuenta;",
    );
  return result.recordset[0] || null;
}
...
export async function updateEstadoCuenta(idCuenta, idUsuario, estado) {
  ...
  const result = await pool
    ...
    .query(
      "UPDATE Cuenta_Bancaria SET estado = @estado WHERE id_cuenta = @id_cuenta AND id_usuario = @id_usuario;",
    );
...
export async function updateSaldoCuenta(
  ...
  const result = await request
    .input("id_cuenta", sql.Int, idCuenta)
    .input("monto", sql.Decimal, monto).query(`
        UPDATE Cuenta_Bancaria 
        SET saldo_actual = saldo_actual ${operacion} @monto 
        WHERE id_cuenta = @id_cuenta ${operacion !== "-" ? "AND saldo_actual >= @monto" : ""};
    `);
  return result.rowsAffected[0] > 0;
}
